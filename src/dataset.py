...
class BertDataset(Dataset):
    ...
    def __getitem__(self, idx):
      ...
      return one_hot_labels, ids, offsets, start_char_idxs, end_char_idxs, one_hot_spans
...
def collate(batch):
  ...
  labels = torch.stack([l for l,_,_,_,_,_ in batch])
  ids = [i for _,i,_,_,_,_ in batch]
  offsets = [o for _,_,o,_,_,_ in batch]
  start_char_idxs = [s for _,_,_,s,_,_  in batch]
  end_char_idxs = [e for _,_,_,_,e,_ in batch]
  n_spans = torch.stack([n for _,_,_,_,_,n in batch])
...
def create_train_val_test(df_sentences, bert_path, labels_col = 'labels_unique'):
  ...
  df_train_val = pd.DataFrame(list(zip(X_train_val, y_train_val, starts_train_val, ends_train_val, spans_train_val)),
                columns =['sentences', 'labels', 'start_idxs', 'end_idxs', 'n_spans'])
  ...
  df_train = pd.DataFrame(list(zip(X_train, y_train, starts_train, ends_train, spans_train)),
                columns =['sentences', 'labels', 'start_idxs', 'end_idxs', 'n_spans'])
  df_val = pd.DataFrame(list(zip(X_val, y_val, starts_val, ends_val, spans_val)),
                columns =['sentences', 'labels', 'start_idxs', 'end_idxs', 'n_spans'])
  df_test = pd.DataFrame(list(zip(X_test, y_test, starts_test, ends_test, spans_test)),
                columns =['sentences', 'labels', 'start_idxs', 'end_idxs', 'n_spans'])
