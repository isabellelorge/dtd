...
class SpanClassifier(nn.Module):
    def __init__(self, batch_size, num_labels, bert_path, dropout_rate, max_spans=10):
        super().__init__()
    ...
    def forward(self, input_ids, masks, segs):
        ...
        for idx, n_spans in enumerate(all_n_spans):
          if n_spans <=1: # also extract span when unique?
            span_logits = spans_logits[idx] # if unique label, classify full sentence
          else:
            ...
            if span_vectors:
              ...
              span_logits,_ = torch.max(span_logits, dim=0)  # TAKING THE MAX IS BEST OPTION (it gets the logit from where the span was correctly predicted if it was anywhere)
            
            else:
              print('NO VALID SPANS')
              span_logits = torch.zeros(self.num_labels) # dummy probabilities in case there are no valid spans

          all_span_logits.append(span_logits)  # this should be batch * labels
          span_logits_tensor = torch.stack(all_span_logits) # create tensor 

        return start_logits, end_logits, span_logits_tensor, n_spans_logits
